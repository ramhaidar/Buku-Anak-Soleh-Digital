...
@section('content_3')
    <div class="p-0 m-0">
        <div class="text-center p-0 m-0">
            <h2 class="text-center mb-4">Detail Catatan Harian Siswa {{ $activityNote->student->user->name }}</h2>
        </div>
            ...
            <div class="p-4 rounded w-75">
                <form action="#" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control rounded-5 border-dark border-2" id="hari_tanggal" name="hari_tanggal" type="text" value="{{ \Carbon\Carbon::parse($activityNote->time_stamp)->format('d-m-Y') }}" readonly disabled>
                    </div>
                    ...
                    <div class="d-flex justify-content-center align-items-stretch pt-3 gap-3">
                        <a class="btn btn-secondary rounded-5 flex-fill d-flex justify-content-center align-items-center text-center px-3 py-2" href="{{ route('teacher.catatan-harian-siswa.index', ['id' => $activityNote->student->id]) }}">
                            Batal
                        </a>
                        @if (is_null($activityNote->parent_question))
                            <a class="btn btn-success rounded-5 flex-fill d-flex justify-content-center align-items-center text-center px-3 py-2 disabled">
                                Jawab Pertanyaan Orang Tua
                            </a>
                        @else
                            <a class="btn btn-success rounded-5 flex-fill d-flex justify-content-center align-items-center text-center px-3 py-2" href="{{ route('teacher.jawaban-pertanyaan-orangtua.index', ['id' => $activityNote->id]) }}">
                                Jawab Pertanyaan Orang Tua
                            </a>
                        @endif
                    </div>
