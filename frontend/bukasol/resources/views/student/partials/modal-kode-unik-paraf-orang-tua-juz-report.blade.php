...
<script>
    var parentSignReportId;

    function showParentCodeModal(reportId) {
        parentSignReportId = reportId;
    ...
    document.getElementById('kodeUnikForm').addEventListener('submit', function(event) {
        ...
        const url = `{{ route('juz-report.parent-sign', ':id') }}`.replace(':id', parentSignReportId);
        ...
        .then(data => {
            ...
            if (data.success) {
                window.showAlert(data.success, true, '#laporanBacaanJuzSiswaTable');
            } else if (data.error) {
                window.showAlert(data.error, false, '#laporanBacaanJuzSiswaTable');
            }
        })
        .catch(error => {
            const parentSignConfirmationModal = bootstrap.Modal.getInstance(document.getElementById('kodeUnikModal'));
